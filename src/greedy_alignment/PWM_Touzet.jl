...
=#
best_score(pwm::AbstractArray{T,2}) where {T <: Real} = @inbounds sum(maximum(@view pwm[:,i]) for i = 1:size(pwm,2));
...
=#
worst_score(pwm::AbstractArray{T,2}) where {T <: Real} = @inbounds sum(minimum(@view pwm[:,i]) for i = 1:size(pwm,2));
...
=#
min_score_range(pwm) = 
    @inbounds pwm[:,sortperm([best_score(@view pwm[:,i])-worst_score(@view pwm[:,i]) 
...
calc_E(pwm, pwm_rounded) = 
    @inbounds sum(maximum((@view pwm[:,i])-(@view pwm_rounded[:,i])) for i = size(pwm));
...
=#
create_Q(m) = 
    Dict{Int16, SortedDict{Double64,Double64}}(i==0 ? i=>SortedDict(0=>1) : i=>SortedDict() for i=0:m);
...
Q_sum(Q_m::SortedDict{Double64,Double64}) = sum(values(Q_m));
...
function pvalue2score(pwm::Matrix{T}, 
    ...
    @inbounds alpha = find_largest_alpha(Q[m], pval_double64);
