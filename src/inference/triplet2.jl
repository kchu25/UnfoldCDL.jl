...
function get_n_range(cartesian_array, N)
    # given a cartesian_array, find the range of indices that 
    # indicate each sequence
...
function get_indicator_arrays(Z_nz, thresh)
    ...
    # @time cartesian_array = reduce(vcat, [([i[1] i[2] i[3]]) for i in found]);
    ...
    @inbounds for vtr in N_valid_ranges[n]
        for i = vtr[1]:vtr[2]-2
            ...
            for j = i+1:vtr[2]-1
                ...
                # d23 = c[c3][1] - c[c2][1];
                # (d12 > 0 && d13 > 0 && d23 > 0) && (H[m1,m2,m3,d12+1,d13+1] += 1)
...
function get_number_of_outliers_via_esd(H; 
                                        ...
                                        r=1000, 
                                        alpha=0.01)
    #= Find all triplets whose count is more than count-thresh, 
       perform generalized ESD test to determine the number of 
       outliers, use this number to determine the count-threshold.
       Returns the cartesian indices in enriched hypercube H
...
function pos_incr!(pos, ind, flen, m1, m2, m3, d12, d13, n, d)
    if haskey(d[n], m1) && haskey(d[n], m2) && haskey(d[n], m3)
        @inbounds for m1_ind in d[n][m1]
...
function obtain_positions(H, found, flen, d, N)
    ...
    @inbounds for (ind,c) in enumerate(found)
        # ind % 10000 == 0 && (@info "$(round(ind/length(found) * 100,digits=2))% done" )
        # c[1] is m1, c[2] is m2, c[3] is m3, c[4] is d12, c[5] is d13
